module "tenant_authorizer_lambda" {
  ...
  lambda_source        = "${path.root}/../../code/release/lambda_tenant_authorizer.zip"
  lambda_policy_list = {
  ...
  lambda_env_variables = {
    ...
    VALID_ISSUERS = "[\"https://python.blueprint.auth.com\"]"
    JWKS_STRING = <<EOF
{
  "keys": [
    {
...
EOF
  ...
  lambda_handler     = "tenant_authorizer.lambda_handler"
